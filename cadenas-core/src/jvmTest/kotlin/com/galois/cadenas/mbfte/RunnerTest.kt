...
internal class RunnerTest {
    ...
    fun testRoundTrip() {
        ...
        println(coverValue)
        assertNotNull(coverValue, "Failed to encode message")
    ...
    @Test
    fun test10() {
        val metrics = runTestFile("strings-10.txt")
        if (metrics != null) {
            assertEquals(0, metrics.failedCount)
        } else {
            fail("Could not get metrics")
        }
    }

    @Test
    fun test100() {
        val metrics = runTestFile("strings-100.txt")
        if (metrics != null) {
            assertEquals(0, metrics.failedCount)
        } else {
            fail("Could not get metrics")
    ...
    @Test
    fun test125() {
        val metrics = runTestFile("strings-125.txt")
        if (metrics != null) {
            assertEquals(0, metrics.failedCount)
        } else {
            fail("Could not get metrics")
    ...
    @Test
    fun test250() {
        val metrics = runTestFile("strings-250.txt")
        if (metrics != null) {
            assertEquals(0, metrics.failedCount)
        } else {
            fail("Could not get metrics")
